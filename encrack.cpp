...
{
    printf("Usage:\n");
    printf("%s -i <input_file> -m <matcher_name> -w <word_list> [-c <ciphers_list>] [-l <decrypt_limit>] [-t <num_threads>] [-o <param=value> -o <param=value> ...] \n", progname);
    ...
    printf("    -l <decrypt_[imit>: Limit decryption to nr of bytes\n");
    printf("                        (for block ciphers this will be rounded up to the block size)\n");
...
int main(int argc, char** argv)
{
    ...
    int numThreads = 4;
    size_t decryptLimit = 0;

    while ((i = getopt(argc, argv, "i:w:t:c:m:o:l:hs")) != -1) {
        switch(i) {
            case 'i':
                inputFile = optarg;
                break;
    ...
    try {
        ...
        subject.setDecryptLimit(decryptLimit);
